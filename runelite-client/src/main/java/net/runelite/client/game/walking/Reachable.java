...
import net.runelite.client.game.walking.CustomLocatable;
...
public class Reachable {
    ...
    public static List<WorldPoint> getNeighbours(Client client, WorldPoint current, CustomLocatable targetObject) {
    ...
    public static List<WorldPoint> getVisitedTiles(Client client, WorldPoint destination, CustomLocatable targetObject) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (current.equals(destination))
                return visitedTiles;
            List<WorldPoint> neighbours = (List<WorldPoint>)getNeighbours(client, current, targetObject).stream().filter(x -> (!visitedTiles.contains(x) && !queue.contains(x))).collect(Collectors.toList());
            queue.addAll(neighbours);
        }
        return visitedTiles;
    }

    public static boolean isInteractable(Client client, CustomLocatable locatable) {
    ...
    public static WorldArea getWorldArea(Client client, GameObject gameObject) {
        ...
        if (!localLocation.isInScene())
            return null;
        LocalPoint minTile = new LocalPoint(localLocation.getX() - x * 128 / 2, localLocation.getY() - y * 128 / 2);
        LocalPoint maxTile = new LocalPoint(localLocation.getX() + x * 128 / 2, localLocation.getY() + y * 128 / 2);
        return createNewWorldArea(
                WorldPoint.fromLocal(client, minTile),
                WorldPoint.fromLocal(client, maxTile));
    }

    public static WorldArea createNewWorldArea(WorldPoint swLocation, WorldPoint neLocation) {
